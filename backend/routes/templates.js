 ...
 * - generateEmbeddingForDescription 失败时直接抛错（禁止降级为无向量模板）
...
// 为模板描述生成 embedding；失败返回 null（审查时降级为关键词匹配）
const generateEmbeddingForDescription = async (description) => {
    if (!description) return null;
    try {
        const vector = await embedText(description);
        return Array.isArray(vector) && vector.length ? JSON.stringify(vector) : null;
    } catch (error) {
        // 静默失败：创建模板仍可用，匹配走关键词
        return null;
    }
