...
async function resetAndRebuildDatabase() {
  ...
  try {
    ...
    await ensureColumn('contracts', 'decision', (table) => table.string('decision'));
    await ensureColumn('contracts', 'decision_at', (table) => table.timestamp('decision_at'));
    await ensureColumn('contracts', 'decision_payload', (table) => table.text('decision_payload'));
    await ensureColumn('contracts', 'playbook_id', (table) => table.string('playbook_id'));
    await ensureColumn('contracts', 'playbook_version', (table) => table.string('playbook_version'));
    await ensureColumn('contracts', 'source', (table) => table.string('source').defaultTo('web'));
    await ensureColumn('contracts', 'external_id', (table) => table.string('external_id').index());
    await ensureColumn('contracts', 'source_meta', (table) => table.text('source_meta'));
    await ensureColumn('contracts', 'confirmation_status', (table) => table.string('confirmation_status').defaultTo('none'));
    await ensureColumn('contracts', 'confirmed_at', (table) => table.timestamp('confirmed_at'));
    await ensureColumn('contracts', 'confirmed_by', (table) => table.string('confirmed_by'));

    const hasReviewAuditEvents = await db.schema.hasTable('review_audit_events');
    if (!hasReviewAuditEvents) {
      console.log('[DB Init] Creating new `review_audit_events` table...');
      await db.schema.createTable('review_audit_events', (table) => {
        table.increments('id').primary();
        table.integer('contract_id').unsigned().notNullable().index();
        table.string('event_type').notNullable().index();
        table.string('actor').defaultTo('system');
        table.string('playbook_id');
        table.string('decision');
        table.text('rule_ids');
        table.text('payload');
        table.timestamp('created_at').defaultTo(db.fn.now());
      });
      console.log('[DB Init] New `review_audit_events` table created successfully.');
    }
