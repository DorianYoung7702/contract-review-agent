...
const { COLLECTION_NAME, VECTOR_FIELD, MILVUS_CONNECT_TIMEOUT_MS, isKeywordOnlyMode, state } = require('./config');
...
const getMilvusClient = async () => {
    ...
    // VECTOR_STORE=pg|postgres|sqlite|relational 强制走关系库；缺省/milvus 才连 Milvus
    if (!MilvusClient || isKeywordOnlyMode(vectorStore) || ['sqlite', 'postgres', 'pg', 'relational'].includes(vectorStore)) return null;
...
const milvusVectorSearch = async (queryVector, { limit, sourceTypes, includeHistorical }) => {
    ...
    try {
        ...
        const mode = String(process.env.VECTOR_STORE || 'milvus').toLowerCase();
        if (!['sqlite', 'postgres', 'pg', 'relational'].includes(mode)) {
            throw new Error(`Milvus search failed: ${error.message}`);
        }
        console.warn(`[Milvus] Search failed: ${error.message}. Relational path may be used.`);
        return null;
    }
};
