...
const { checkAmountArithmetic } = require('../amountArithmeticCheck');
...
const { normalizeAnalysisResult, aggregateClauseResults, buildStandardComparison, buildStructuredResult } = require('./analysisCore');
const { analyzeSealAndSignature } = require('./seal');
const { matchPlaybook } = require('../playbook/matcher');
const { decideApproval } = require('../approvalEngine');
const { writeAuditEvent } = require('../auditService');
const { markAwaitingHumanConfirm } = require('../dingtalk/confirmation');
const { createContractVersionSnapshot } = require('./version');
const {
    isYunjiaCompany,
    getYunjiaFixedProfile,
} = require('./creditCodeAutofill');
...
const runAnalysisInBackground = async (contractId, userId, userPerspective, preAnalysisData) => {
    ...
    try {
        ...
        // Snapshot "before review" so UI can open pre-annotation version
        let preReviewVersion = null;
        try {
            preReviewVersion = await createContractVersionSnapshot(contract, 'pre-review');
        } catch (snapErr) {
            console.warn('[Analysis] pre-review snapshot failed:', snapErr.message);
        }
        ...
        const earlyPlaybook = matchPlaybook(preAnalysisData.contract_type || template?.name || '');
        preAnalysisData.playbook_id = earlyPlaybook?.playbook_id || null;
        preAnalysisData.playbook_version = earlyPlaybook?.version || null;
        preAnalysisData._playbook_rules_for_prompt = (earlyPlaybook?.rules || [])
            .map((r) => `${r.id} | ${r.name} | ${r.severity} | ${r.requirement}`)
            .join('\n') || '无';
        ...
        const companyNames = extractCompanyNames(plainText)
            .filter((name) => !/^(?:甲方|乙方|双方|本方|我方|贵方|对方)(?:公司)?$/.test(name))
            .filter((name) => !/(?:从|向|为)(?:甲方|乙方)公司/.test(name))
            .slice(0, 3);
        const companySearchResults = await Promise.all(
            companyNames.map(async (name) => {
                try {
                    // 云伽资料由本方已确认配置固定提供；只对交易对方发起工商查询。
                    const profile = isYunjiaCompany(name)
                        ? getYunjiaFixedProfile()
                        : await verifyCompany(name);
                    return {
                        companyName: name,
        ...
        // 工商信息仅用于主体核验和审查报告，不自动修改合同正文。
        const creditCodeAutofill = {
            enabled: false,
            applied: [],
            skipped: [{ reason: 'DISABLED_BY_REVIEW_POLICY' }],
        };
        await emitAnalysisProgress(null, contractId, {
            step: 'credit_code_autofill',
            status: 'completed',
            message: '工商信息仅用于主体核验，不写入合同正文。',
            partialResult: { credit_code_autofill: creditCodeAutofill },
        });
...
注册地址: ${company.profile.registered_address || '未知'}
...
数据来源: ${company.profile.data_source}`;
        ...
        // Step 3.5: 硬性合规检查（规则引擎）+ 金额算术一致性（数量×单价）
        ...
        try {
            const arith = checkAmountArithmetic(plainText);
            if (arith.violations.length) {
                hardViolations = [...hardViolations, ...arith.violations];
            }
        } catch (arithErr) {
            console.warn('[AmountArithmetic] 金额算术检查失败:', arithErr.message);
        }
...
输出 JSON 结构：
{
  "dispute_points": [{"title":"风险标题","original_clause":"合同原文","legal_reference":"依据","dispute_rationale":"风险说明","plain_language":"大白话说明","severity":"高/中/低","rule_id":"PLAYBOOK规则ID或null","current_value":"合同现值","standard_value":"标准要求"}],
  "missing_clauses": [{"title":"缺失条款","description":"为什么缺失","suggested_clause":"可补充条款","rule_id":"PLAYBOOK规则ID或null","severity":"高/中/低"}],
  "party_review": [{"title":"主体审查项","description":"审查结论","plain_language":"大白话说明"}],
  "modification_suggestions": [{"title":"建议标题","original_text":"合同中可定位的完整原文句子或段落","suggested_text":"可直接替换 original_text 的完整文本","reason":"修改理由","plain_language":"大白话说明","anchor_hint":"用于定位的短语","rule_id":"PLAYBOOK规则ID或null","severity":"高/中/低","current_value":"合同现值","standard_value":"标准要求"}],
  "breach_cost_analysis": [{"scenario":"违约场景","legal_basis":"依据","estimated_cost":"预计成本"}]
}

Playbook 规则清单（若风险命中请尽量填写 rule_id，未命中填 null）：
${(preAnalysisData._playbook_rules_for_prompt || '无')}
...
- suggested_text 必须在 original_text 上做最小必要改动，保留与风险无关的原词句和顺序，禁止仅为润色而整句改写；系统将据此生成字词级红线。
...
- 若明细中同时出现数量、单价与金额/合计，必须核对「数量×单价是否等于金额」；不一致的应给出修改建议（改金额或改数量/单价使三者一致）。
...
---`;
        ...
        analysisResult.credit_code_autofill = creditCodeAutofill;
            ...
            analysisResult.company_review = companySearchResults.map((company) => {
                if (company.profile) {
                    ...
                    return {
                        ...
                        evidence_summary: `统一社会信用代码:${company.profile.unified_code || '未知'};注册地址:${company.profile.registered_address || '未知'};法定代表人:${company.profile.legal_representative || '未知'};企业状态:${company.profile.company_status || '未知'}`,
        ...
        // Step 6: Playbook 匹配 + Approval Engine + 保存结果
        await emitAnalysisProgress(null, contractId, { step: 'finalize', status: 'running', message: '正在保存审查结果并生成审批决策...' });

        const playbook = matchPlaybook(preAnalysisData?.contract_type || analysisResult.template?.name || '');
        const structuredResult = buildStructuredResult(analysisResult, hardViolations);
        structuredResult.contract_type = preAnalysisData?.contract_type || structuredResult.contract_type;

        let contractAmount = null;
        try {
            const amountMatch = String(plainText || '').match(/(?:合同金额|总价|价款)[^\d]{0,12}(\d+(?:\.\d+)?)\s*(万元|元)?/);
            if (amountMatch) {
                contractAmount = Number(amountMatch[1]) * (amountMatch[2] === '万元' ? 10000 : 1);
            }
        } catch { /* ignore */ }
        ...
        const approval = decideApproval({
            ...
            contractProfile: {
                ...
                contract_type: structuredResult.contract_type,
            },
        });

        analysisResult.structured_result = structuredResult;
        analysisResult.approval = approval;
        analysisResult.playbook = playbook
            ? { playbook_id: playbook.playbook_id, version: playbook.version, contract_type: playbook.contract_type }
            : null;

        // Annotation targets for OnlyOffice comments (all decisions; prefer locatable original text)
        const annotationCandidates = [];
        const pushAnno = (item, severityFallback = 'MEDIUM') => {
            if (!item || typeof item !== 'object') return;
            const severity = String(item.severity || severityFallback).toUpperCase();
            const original = item.original_text || item.original_clause || item.current_value || '';
            annotationCandidates.push({
                rule_id: item.rule_id || null,
                title: item.title || item.description || '审查意见',
                severity,
                comment: `${item.title || item.description || '风险'}：${item.reason || item.dispute_rationale || item.description || ''}${item.suggestion || item.suggested_text || item.suggested_clause ? `；建议：${item.suggestion || item.suggested_text || item.suggested_clause}` : ''}`,
                original_text: original,
            });
        };
        (analysisResult.modification_suggestions || []).forEach((x) => pushAnno(x, 'MEDIUM'));
        (analysisResult.dispute_points || []).forEach((x) => pushAnno(x, 'HIGH'));
        (analysisResult.hard_violations || []).forEach((x) => pushAnno({
            ...x,
            title: x.description || x.title,
            original_text: x.contract_value || x.original_text,
            reason: x.description,
            suggestion: x.fix_template,
        }, 'HIGH'));
        (approval.findings || []).forEach((x) => pushAnno(x, x.severity || 'MEDIUM'));

        const seenAnno = new Set();
        analysisResult.auto_annotation_targets = annotationCandidates
            .filter((t) => {
                const key = `${t.rule_id || ''}|${t.title}|${String(t.original_text).slice(0, 40)}`;
                if (seenAnno.has(key)) return false;
                seenAnno.add(key);
                return true;
            })
        ...
        analysisResult.version_meta = {
            pre_review_version_id: preReviewVersion?.id || null,
            pre_review_version_no: preReviewVersion?.version_no || null,
            annotated_version_id: null,
            annotated_version_no: null,
        };
        ...
        await writeAuditEvent({
            ...
            decision: approval.decision,
            ruleIds: approval.matched_rule_ids?.length
                ? approval.matched_rule_ids
                : approval.passed_rule_ids,
            payload: {
                risk_score: approval.risk_score,
                reason_codes: approval.reason_codes,
                summary: approval.summary,
                matched_rule_ids: approval.matched_rule_ids,
                passed_rule_ids: approval.passed_rule_ids,
            },
        });

        // DingTalk (and any source that opts in): AI decision then wait for human confirm
        const contractRow = await db('contracts').where({ id: contractId }).select('source').first();
        if (contractRow?.source === 'dingtalk') {
            try {
                const confirmInfo = await markAwaitingHumanConfirm(contractId);
                analysisResult.confirmation_status = confirmInfo?.confirmation_status || 'pending';
            } catch (confirmErr) {
                console.error('[DingTalk] markAwaitingHumanConfirm failed:', confirmErr.message);
            }
        }

        updateAnalysisJob(contractId, { status: 'completed', result: analysisResult, percent: 100 });
        await emitAnalysisProgress(null, contractId, { step: 'finalize', status: 'completed', message: `审查结果已保存，审批决策：${approval.decision}`, partialResult: analysisResult });
        if (getIoInstance()) getIoInstance().to(`contract-${contractId}`).emit('analysis-complete', { results: analysisResult, perspective: userPerspective, approval });
    } catch (error) {
