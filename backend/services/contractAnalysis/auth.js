...
/** Owner or shared DingTalk-sourced contracts (legal desk visibility). */
const findAccessibleContract = async (id, userId) => {
    const owned = await findOwnedContract(id, userId);
    if (owned) return owned;
    return db('contracts').where({ id, source: 'dingtalk' }).first();
};
