...
/** Collect annotation lines for appendix (comments, not body rewrites). */
const collectAnnotationItems = (reviewData = {}) => {
    const fromTargets = Array.isArray(reviewData.auto_annotation_targets)
        ? reviewData.auto_annotation_targets.map((t, index) => ({
            index: index + 1,
            title: t.title || t.rule_id || `批注 ${index + 1}`,
    ...
    const suggestions = Array.isArray(reviewData.modification_suggestions)
        ? reviewData.modification_suggestions
        : [];
    return suggestions.map((item, index) => ({
        index: index + 1,
        title: item.title || item.clause || `修改建议 ${index + 1}`,
        original: item.original_text || item.original_clause || '',
        comment: [
            item.suggested_text || item.modification
                ? `建议修改为：${item.suggested_text || item.modification}`
                : '',
            item.reason || item.rationale ? `理由：${item.reason || item.rationale}` : '',
        ].filter(Boolean).join('\n'),
    }));
};

/**
 * Build a PDF buffer that lists review comments as an appendix
 * (used when Word comments may not survive DOCX→PDF conversion).
 */
const buildAnnotationAppendixPdf = (contract, reviewData = {}) => new Promise((resolve, reject) => {
    const doc = new PDFDocument({ margin: 48, size: 'A4' });
    const chunks = [];
    ...
    doc.fontSize(16).text('合同审查批注附录');
    doc.moveDown(0.4).fontSize(10)
        .text('说明：本 PDF 正文来自「审查后批注版」快照（保留原文，不以一键采纳改写正文）。')
        .text('若转换后正文内批注气泡不可见，请以下表意见为准。');
    doc.moveDown(0.3).text(`文件：${contract.original_filename || ''}`);
    doc.text(`导出时间：${new Date().toLocaleString('zh-CN')}`);
    if (contract.perspective) doc.text(`审查立场：${contract.perspective}`);
    if (reviewData.approval?.decision) doc.text(`AI 决策：${reviewData.approval.decision}`);

    const items = collectAnnotationItems(reviewData);
    doc.moveDown().fontSize(13).text(`批注清单（共 ${items.length} 条）`);
    if (!items.length) {
        doc.moveDown(0.4).fontSize(10).text('暂无批注条目。');
    } else {
        items.forEach((item) => {
            doc.moveDown(0.6).fontSize(11).text(`#${item.index} ${item.title}`);
            if (item.original) {
                doc.fontSize(9).fillColor('#333333')
                    .text(`原文锚点：${String(item.original).slice(0, 800)}`);
            }
            if (item.comment) {
                doc.fontSize(9).fillColor('#111111')
                    .text(`意见：${String(item.comment).slice(0, 1200)}`);
            }
...
module.exports = {
    ...
    collectAnnotationItems,
    buildAnnotationAppendixPdf,
};
