...
const toRelevantKnowledgeItem = (item) => ({
    ...
    retrieval_engine: item.retrieval_engine || null,
    embedding_backend: item.embedding_backend || null,
});
...
const getRelevantKnowledge = async (options, limit = 8) => {
    ...
    const merged = mergeChannelsWithConfidence(channelA, channelB, limit).map(toRelevantKnowledgeItem);
    if (process.env.RAG_METRICS === '1' || process.env.RAG_METRICS === 'true') {
        console.log('[RAG]', JSON.stringify({
            latency_ms: Date.now() - startedAt,
            limit,
            channelA_queries: channelAQueries.length,
            channelB_queries: channelBQueries.length,
            channelA_hits: channelA.length,
            channelB_hits: channelB.length,
            merged: merged.length,
            contractType: options.contractType || '',
            engines: [...new Set(merged.map((m) => m.retrieval_engine).filter(Boolean))],
        }));
    }
    return merged;
};
