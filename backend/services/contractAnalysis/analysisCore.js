...
const normalizeFindingItem = (item = {}) => ({
    ...
    severity: normalizeSeverityLabel(item.severity || item.risk_level || item.risk),
...
const normalizeAnalysisResult = (result, plainText = '') => ({
    dispute_points: Array.isArray(result.dispute_points)
        ? result.dispute_points.map((dp) => ({
            ...normalizeFindingItem(dp),
            evidence: buildEvidence(dp, plainText, result.relevant_laws || []),
        }))
        : [],
    missing_clauses: Array.isArray(result.missing_clauses)
        ? result.missing_clauses.map((mc) => normalizeFindingItem({ ...mc, severity: mc.severity || 'high' }))
        : [],
    party_review: Array.isArray(result.party_review) ? result.party_review : [],
    modification_suggestions: Array.isArray(result.modification_suggestions)
        ? result.modification_suggestions.map((ms) => normalizeFindingItem(ms))
        : [],
...
/**
 * Merge hard_violations into a unified structured_result used by Approval Engine.
 */
const buildStructuredResult = (analysisResult = {}, hardViolations = []) => {
    const hardFindings = (hardViolations || []).map((v) => normalizeFindingItem({
        title: v.description || v.title || '硬性规则违规',
        rule_id: v.rule_id,
        severity: v.severity || 'high',
        reason: v.description || v.legal_basis || '',
        current_value: v.contract_value,
        standard_value: v.limit_value,
        suggestion: v.fix_template || '',
        legal_reference: v.legal_basis,
        forbidden: Boolean(v.forbidden),
        auto_decision: v.auto_decision || null,
    }));

    return {
        contract_type: analysisResult.contract_type || analysisResult.template?.name || '',
        overall_risk: analysisResult.overall_risk || null,
        dispute_points: analysisResult.dispute_points || [],
        missing_clauses: analysisResult.missing_clauses || [],
        modification_suggestions: analysisResult.modification_suggestions || [],
        hard_violations: hardFindings,
        findings: [
            ...(analysisResult.dispute_points || []),
            ...(analysisResult.missing_clauses || []).map((x) => ({ ...x, missing: true })),
            ...hardFindings,
        ],
    };
};
