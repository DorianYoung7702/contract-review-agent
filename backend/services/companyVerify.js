...
const verifyByTiyanji = async (name) => {
    ...
    const profile = {
        ...
        registered_address: data.regLocation || data.registeredAddress || data.regAddress || data.address || '',
...
const verifyByQichacha = async (name) => {
    ...
    const profile = {
        ...
        registered_address: payload.Address || payload.RegAddress || payload.RegisteredAddress || payload.address || '',
...
const buildProfileFromWebSearch = async (name, webResult) => {
    ...
    // 注册地址/住所。网页摘要只作为回退证据，输出时仍保留 data_source 供人工核对。
    ...
    for (const text of candidateTexts) {
        ...
        if (!registered_address) registered_address = extractField(text, addressPatterns)
            .replace(/(?:法定代表人|法人|注册资本|成立日期|经营状态)[：:].*$/, '')
            .trim();
        ...
        // 如果所有字段都已获取，可提前退出
        if (unified_code && registered_address && legal_representative && registered_capital && establish_date && company_status) break;
    }
