...
import api from '../api';
import { resolveDefaultReviewPerspective } from '../utils/reviewPerspective';
...
export function useReviewSession(state, deps) {
    ...
    const loadContractFromServer = async (contractId) => {
        ...
        try {
            ...
            contract.confirmation_status = contractData.contract?.confirmation_status || 'none';
            contract.source = contractData.contract?.source || 'web';
            contract.source_meta = contractData.contract?.source_meta || null;
            ...
            perspective.value = resolveDefaultReviewPerspective({
                explicitPerspective: contractData.perspective,
                serverDefault: preAnalysisData.default_perspective,
                parties: allPotentialParties.value,
            });
            ...
            if (contractData.approval) {
                reviewData.approval = contractData.approval;
            }
            if (!reviewData.playbook && (contractData.contract?.playbook_id || contractData.approval?.playbook_id)) {
                reviewData.playbook = {
                    playbook_id: contractData.contract?.playbook_id || contractData.approval?.playbook_id,
                    version: contractData.contract?.playbook_version || contractData.approval?.playbook_version,
                };
            }
    ...
    const restoreSessionFromSavedState = async (savedState) => {
        ...
        perspective.value = resolveDefaultReviewPerspective({
            explicitPerspective: savedState.perspective,
            serverDefault: preAnalysisData.default_perspective,
            parties: allPotentialParties.value,
        });
