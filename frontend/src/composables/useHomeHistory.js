...
export function useHomeHistory() {
    ...
    const filteredHistory = computed(() => {
        ...
        return history.value.filter((item) => {
            ...
            if (decisionFilter.value && item.decision !== decisionFilter.value) return false;
            if (sourceFilter.value && (item.source || 'web') !== sourceFilter.value) return false;
            if (confirmationFilter.value && (item.confirmation_status || 'none') !== confirmationFilter.value) return false;
            return true;
        });
    });
    ...
    // 筛选条件变化时回到第一页
    watch([searchKeyword, statusFilter, typeFilter, decisionFilter, sourceFilter, confirmationFilter], () => { historyPage.value = 1; });
...
export const confirmationText = (status) => ({
    none: '—',
    pending: '待确认',
    confirmed: '已确认',
    rejected: '已驳回确认',
