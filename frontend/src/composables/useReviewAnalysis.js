...
export function useReviewAnalysis(state, deps = {}) {
    const {
    ...
    const { applyAutoAnnotations } = deps;
        ...
        statusPollTimer.value = setInterval(async () => {
            ...
            try {
                ...
                if (data.status === 'completed' && data.result) {
                    ...
                    if (data.result.approval) reviewData.approval = data.result.approval;
                    ...
                    if (typeof applyAutoAnnotations === 'function') {
                        setTimeout(() => applyAutoAnnotations(), 1500);
                    }
