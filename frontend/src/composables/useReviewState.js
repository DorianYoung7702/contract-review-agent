...
export function useReviewState({ isResetting }) {
    ...
    const approvalDecision = computed(() => reviewData.approval || null);
    const decisionBadgeClass = computed(() => {
        const d = approvalDecision.value?.decision;
        if (d === 'PASS') return 'bg-green-100 text-green-800 border-green-300';
        if (d === 'REJECT') return 'bg-red-100 text-red-800 border-red-300';
        if (d === 'MANUAL') return 'bg-amber-100 text-amber-800 border-amber-300';
        return 'bg-gray-100 text-gray-700 border-gray-300';
    });
    const decisionLabel = computed(() => {
        const d = approvalDecision.value?.decision;
        if (d === 'PASS') return '自动通过';
        if (d === 'REJECT') return '自动驳回';
        if (d === 'MANUAL') return '转人工法务';
        return '待决策';
    });
    ...
    return {
        ...
        approvalDecision, decisionBadgeClass, decisionLabel,
        saveState, resetState,
    };
}
