  ...
  <div>
    <div v-if="reviewData.modification_suggestions && reviewData.modification_suggestions.length > 0" class="mb-3 flex items-center justify-between gap-2">
      ...
      <div class="flex items-center gap-2">
        ...
        <span v-else-if="!canAdoptSuggestions()" class="text-xs text-amber-600">待对方确认红线后再采纳</span>
        <button @click="applySelectedSuggestions" :disabled="batchApplying || isPdfContract || selectedSuggestionIndexes.length === 0 || !canAdoptSuggestions()" class="px-3 py-1.5 text-xs font-medium text-white bg-primary rounded hover:bg-primary-dark disabled:opacity-50 disabled:cursor-not-allowed">
    ...
    <div v-if="reviewData.modification_suggestions && reviewData.modification_suggestions.length > 0" class="space-y-4">
      <div v-for="(item, index) in reviewData.modification_suggestions" :key="'ms-' + index" class="p-4 bg-bg-subtle rounded-md border border-border-color transition-all hover:shadow-md">
        ...
        <div class="mt-4 pt-3 border-t border-gray-100 flex justify-end items-center">
          ...
          <button @click="adoptSuggestion(item)" :disabled="isPdfContract || item.adopted || !canAdoptSuggestions()" class="px-3 py-1.5 text-xs font-medium text-white bg-primary rounded hover:bg-primary-dark transition-colors flex items-center disabled:opacity-50 disabled:cursor-not-allowed">
            ...
            {{ item.adopted ? '已采纳' : (!canAdoptSuggestions() ? '待对方确认' : '一键采纳建议') }}
          </button>
        </div>
        <!-- 4.1 谈判推演面板 -->
