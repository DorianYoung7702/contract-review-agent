<template>
  <div>
    <!-- 法务审批决策 -->
    <div v-if="approvalDecision" class="mb-4 p-4 rounded-md border-2" :class="decisionPanelClass">
      <div class="flex items-center justify-between gap-3 flex-wrap">
        <div class="flex items-center gap-3">
          <span class="text-sm font-semibold text-text-dark">法务审批决策</span>
          <span class="px-3 py-1 text-sm font-bold rounded-full border" :class="decisionBadgeClass">{{ decisionLabel }}</span>
          <span class="text-xs text-text-light">风险分 {{ approvalDecision.risk_score ?? '—' }}/100</span>
        </div>
        <span v-if="reviewData.playbook" class="text-xs text-text-light">
          Playbook：{{ reviewData.playbook.playbook_id }} v{{ reviewData.playbook.version }}
        </span>
      </div>
      <p class="mt-2 text-sm text-text-main">{{ approvalDecision.summary }}</p>
      <div class="mt-3 grid grid-cols-1 md:grid-cols-2 gap-2 text-xs">
        <div class="p-2 bg-white/70 rounded border border-border-color">
          <p class="font-semibold text-text-dark mb-1">命中规则</p>
          <p class="text-text-main break-all">{{ (approvalDecision.matched_rule_ids || []).join('、') || '无' }}</p>
        </div>
        <div class="p-2 bg-white/70 rounded border border-border-color">
          <p class="font-semibold text-text-dark mb-1">通过/依据规则</p>
          <p class="text-text-main break-all">{{ (approvalDecision.passed_rule_ids || []).slice(0, 12).join('、') || '无' }}</p>
        </div>
      </div>
      <p class="mt-2 text-xs text-text-light">原因码：{{ (approvalDecision.reason_codes || []).join('；') || '—' }}</p>
      <div v-if="contract.source === 'dingtalk' || contract.confirmation_status === 'pending'" class="mt-3 pt-3 border-t border-border-color">
        <div class="flex items-center justify-between gap-2 flex-wrap">
          <div class="text-xs text-text-main">
            <span class="font-semibold">来源：</span>{{ contract.source === 'dingtalk' ? '钉钉' : '网页' }}
            <span v-if="contract.source_meta?.staff_nick" class="ml-2">发起人：{{ contract.source_meta.staff_nick }}</span>
            <span class="ml-2 px-2 py-0.5 rounded border text-xs font-bold" :class="confirmBadgeClass">{{ confirmLabel }}</span>
          </div>
          <div v-if="contract.confirmation_status === 'pending'" class="flex gap-2">
            <button
            ...
            <button
              :disabled="confirmBusy"
              class="px-3 py-1 text-xs font-medium text-white bg-red-600 rounded hover:bg-red-700 disabled:opacity-50"
              @click="submitConfirm('reject')"
            >驳回确认</button>
          </div>
        </div>
      </div>
...
export default {
  ...
  setup() {
    ...
    const decisionPanelClass = computed(() => {
      const d = review.approvalDecision?.value?.decision;
      if (d === 'PASS') return 'bg-green-50 border-green-400';
      if (d === 'REJECT') return 'bg-red-50 border-red-400';
      if (d === 'MANUAL') return 'bg-amber-50 border-amber-400';
      return 'bg-gray-50 border-gray-300';
    });
    const confirmLabel = computed(() => {
      const s = review.contract?.confirmation_status;
      if (s === 'pending') return '等待确认';
      if (s === 'confirmed') return '已确认';
      if (s === 'rejected') return '已驳回确认';
      return '无需确认';
    });
    const confirmBadgeClass = computed(() => {
      const s = review.contract?.confirmation_status;
    ...
    const submitConfirm = async (action) => {
      if (!review.contract?.id) return;
      let reason = '';
      if (action === 'reject') {
        reason = window.prompt('请填写驳回确认原因（可选）') || '';
      }
      confirmBusy.value = true;
      try {
        const res = await api.confirmDingTalkDecision({
          contract_id: review.contract.id,
          action,
          reason,
        });
        review.contract.confirmation_status = res.data.confirmation_status;
        ElMessage.success(action === 'confirm' ? '已确认' : '已驳回确认');
      } catch (err) {
        ElMessage.error(err.response?.data?.error || '确认操作失败');
      } finally {
    ...
    return {
      ...
      approvalDecision: review.approvalDecision,
      decisionBadgeClass: review.decisionBadgeClass,
      decisionLabel: review.decisionLabel,
      decisionPanelClass,
      confirmBusy,
      confirmLabel,
      confirmBadgeClass,
